...
export function RealtimeAnalyticsTab() {
  ...
  async function exportPDF() {
    ...
    try {
      ...
      const doc = new jsPDF();
      const autoTable = (autoTableMod as unknown as { default: (d: unknown, o: unknown) => void })
        .default;
      doc.setFontSize(16);
      ...
      autoTable(doc, {
        ...
        body: data.countries.map((c) => [c.country, String(c.visitors)]),
      });
      autoTable(doc, {
        head: [["Governorate", "Visitors"]],
        body: data.governorates.map((g) => [g.governorate, String(g.visitors)]),
      });
      autoTable(doc, {
        head: [["Source", "Visitors"]],
        body: data.sources.map((s) => [s.source ?? "", String(s.visitors)]),
      });
      autoTable(doc, {
        head: [["Device", "Visitors"]],
        body: data.devices.map((d) => [d.device ?? "", String(d.visitors)]),
      });
      autoTable(doc, {
        head: [["Browser", "Visitors"]],
        body: data.browsers.map((b) => [b.browser ?? "", String(b.visitors)]),
      });
      autoTable(doc, {
        head: [["OS", "Visitors"]],
        body: data.oses.map((o) => [o.os ?? "", String(o.visitors)]),
      });
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          <span className="relative inline-flex h-2.5 w-2.5">
            <span
              className={`absolute inline-flex h-full w-full rounded-full ${live ? "bg-green-500 animate-ping opacity-75" : "bg-muted"}`}
            />
            <span
              className={`relative inline-flex h-2.5 w-2.5 rounded-full ${live ? "bg-green-500" : "bg-muted"}`}
            />
          </span>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button
            onClick={exportCSV}
            className="rounded border border-border px-3 py-1 text-[11px] font-semibold uppercase tracking-widest hover:bg-muted"
          >
      ...
      <Card title="Customer behavior funnel">
        <div className="space-y-2">
          {funnelRows.map((row, i) => {
            const p =
              i === 0 ? 100 : Math.round(((row.value || 0) / (funnelRows[0].value || 1)) * 100);
            const Icon = row.icon;
            return (
        ...
        <div className="mt-3 flex items-center gap-6 text-xs text-muted-foreground">
          <span>
            Cart abandonment: <b className="text-foreground">{cartAbandonment}%</b>
          </span>
          <span>
            View→Order:{" "}
            <b className="text-foreground">
              {pct(data.funnel.orders_completed, data.funnel.product_views)}
            </b>
          </span>
        </div>
      </Card>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Card title={`Live visitors (${data.live_feed.length})`}>
          <div className="max-h-96 space-y-1.5 overflow-y-auto pr-1">
            {data.live_feed.length === 0 && (
              <div className="text-xs text-muted-foreground">No active visitors right now.</div>
            )}
            {data.live_feed.map((v) => (
              <div
                key={v.visitor_hash + v.created_at}
                className="flex items-start gap-2 rounded border border-border/60 bg-card px-2 py-1.5 text-xs"
              >
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="truncate">
                      {[v.city, v.governorate, v.country].filter(Boolean).join(", ") ||
                        "Unknown location"}
                    </span>
                  </div>
        ...
        <Card title="Live searches">
          <div className="max-h-96 space-y-1.5 overflow-y-auto pr-1">
            {data.live_searches.length === 0 && (
              <div className="text-xs text-muted-foreground">No live searches.</div>
            )}
            {data.live_searches.map((s, i) => (
              <div
                key={i}
                className="flex items-center gap-2 rounded border border-border/60 bg-card px-2 py-1.5 text-xs"
              >
        ...
        <Card title="Top countries">
          <RankList
            rows={data.countries.map((c) => ({
              label: `${flag(c.country_code)} ${c.country}`,
              value: c.visitors,
            }))}
          />
        </Card>
        ...
        <Card title="Traffic sources">
          <RankList
            rows={data.sources.map((s) => ({
              label:
                (s.source || "direct").charAt(0).toUpperCase() + (s.source || "direct").slice(1),
              value: s.visitors,
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Card title="Browsers">
          <RankList
            rows={data.browsers.map((b) => ({ label: b.browser || "Other", value: b.visitors }))}
          />
        </Card>
      ...
      <p className="text-[10px] text-muted-foreground">
        Visitor IDs are hashed. No personal information is displayed. Location is estimated from IP
        address at page load.
      </p>
    </div>
  );
}
  ...
  return (
    <div className="space-y-1.5">
      {rows.map((r, i) => (
        <div
          key={i}
          className="relative overflow-hidden rounded border border-border/60 bg-background/40 px-2 py-1.5 text-xs"
        >
          <div
