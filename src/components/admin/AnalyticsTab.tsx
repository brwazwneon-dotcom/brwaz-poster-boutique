...
function fmtShort(d: string) {
  try {
    return new Date(d).toLocaleDateString("en-EG", { month: "short", day: "numeric" });
  } catch {
    return d;
  }
}
...
function computeRange(
  ...
  if (preset === "custom" && cFrom && cTo) {
    return {
      ...
      to: new Date(new Date(cTo).getTime() + 86400000).toISOString(),
    };
  }
  ...
  return (
    ...
    >
      ...
      <div className="relative flex items-start justify-between gap-3">
        ...
        <div className={`grid h-9 w-9 shrink-0 place-items-center rounded-md ${iconBg[accent]}`}>
          {icon}
        </div>
      </div>
    </button>
...
function statusPill(status: string | null) {
  ...
  const cls =
    s === "completed" || s === "delivered" || s === "shipped"
  ...
  return (
    <span
      className={`rounded-sm px-2 py-0.5 text-[10px] font-medium uppercase tracking-widest ${cls}`}
    >
      {s}
    </span>
  );
...
export function AnalyticsTab({ onNavigate }: { onNavigate?: (tab: AdminTab) => void }) {
  ...
  const sourceTotal = Math.max(
    1,
    sourceRows.reduce((a, r) => a + r.value, 0),
  );
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            BRWAZWNEON · live overview · auto-refreshing
          </p>
        </div>
        <div className="flex items-center gap-2">
      ...
      <div className="flex flex-wrap items-center gap-2 rounded-md border border-border bg-card px-3 py-2">
        ...
        <span className="ml-auto text-[11px] text-muted-foreground">
          {fmtNum(data.orders_range)} orders · {fmtEGP(data.revenue_range)} ·{" "}
          {fmtNum(data.visitors_range)} visitors
        </span>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6">
        <KpiCard
          ...
          value={fmtNum(data.orders_today)}
          icon={<ShoppingCart className="h-4 w-4" />}
          accent="info"
          onClick={goto("orders")}
        />
        <KpiCard
          ...
          value={fmtEGP(data.revenue_today)}
        ...
        <KpiCard
          ...
          value={fmtEGP(data.revenue_month)}
          sub="Last 30 days"
        ...
        <KpiCard
          ...
          value={fmtNum(data.orders_pending)}
        ...
        <KpiCard
          ...
          value={fmtNum(data.orders_completed)}
        ...
        <KpiCard
          ...
          value={fmtNum(data.orders_cancelled)}
        ...
        <KpiCard
          ...
          value={fmtNum(data.visitors_today)}
          icon={<Users className="h-4 w-4" />}
          accent="info"
        />
        <KpiCard
          label="Visitors This Month"
          value={fmtNum(data.visitors_month)}
        ...
        <KpiCard
          ...
          sub={`${fmtNum(data.orders_total)} orders / ${fmtNum(data.visitors_total)} visitors`}
        ...
        <div className="rounded-md border border-border bg-card p-4 lg:col-span-2">
          <div className="flex items-center justify-between">
            ...
            <span className="text-[11px] text-muted-foreground">
              {fmtEGP(data.revenue_week)} this week
            </span>
          </div>
      ...
      <div className="grid gap-4 lg:grid-cols-2 xl:grid-cols-4">
        ...
        <Panel title="Most Wishlisted" icon={<Heart className="h-4 w-4 text-muted-foreground" />}>
          <PosterList
            items={data.top_wishlisted}
            metricKey="wishlist_count"
            metricLabel="❤"
            onSeeAll={goto("wishlists")}
          />
        </Panel>
        <Panel
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        ...
        >
          <ul className="divide-y divide-border/60 text-sm">
            {data.top_categories.length === 0 && (
              <li className="py-2 text-xs text-muted-foreground">No data yet.</li>
            )}
            {data.top_categories.map((c) => (
              <li key={c.id} className="flex items-center justify-between gap-2 py-2">
                ...
                <span className="text-[11px] text-muted-foreground">
                  {fmtNum(c.sales)} sold · {fmtNum(c.views)} views
                </span>
              </li>
            ))}
          </ul>
          ...
          {data.top_searches.length === 0 ? (
            ...
            <div className="flex flex-wrap gap-1.5">
              {data.top_searches.slice(0, 16).map((s) => (
                <span
                  key={s.q}
                  className="rounded-sm border border-border bg-background/40 px-2 py-1 text-xs"
                >
        ...
        <Panel
          title="Top Governorates / Cities"
          icon={<MapPin className="h-4 w-4 text-muted-foreground" />}
        >
          <ul className="divide-y divide-border/60 text-sm">
            {data.top_governorates.length === 0 && (
              <li className="py-2 text-xs text-muted-foreground">No orders yet.</li>
            )}
            {data.top_governorates.slice(0, 8).map((g) => (
              <li key={g.governorate} className="flex items-center justify-between gap-2 py-2">
                ...
                <span className="text-[11px] text-muted-foreground">
                  {fmtNum(g.orders)} · {fmtEGP(g.revenue)}
                </span>
              </li>
            ))}
          </ul>
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
        <KpiCard
          label="Frame Orders"
          value={fmtNum(data.conversions.frame_orders)}
          icon={<Frame className="h-4 w-4" />}
          onClick={goto("orders")}
        />
        <KpiCard
          label="Wood Portrait Orders"
          value={fmtNum(data.conversions.wooden_portrait)}
          icon={<Frame className="h-4 w-4" />}
          onClick={goto("orders")}
        />
        <KpiCard
          label="Photo Printing Orders"
          value={fmtNum(data.conversions.photo_printing)}
          icon={<ImgIcon className="h-4 w-4" />}
          onClick={goto("orders")}
        />
        <KpiCard
          label="Custom Design Orders"
          value={fmtNum(data.conversions.custom_design)}
          icon={<ImagePlus className="h-4 w-4" />}
          onClick={goto("custom")}
        />
        <KpiCard
          label="Black Frame Orders"
          value={fmtNum(data.conversions.black_frame)}
          icon={<Frame className="h-4 w-4" />}
        />
        <KpiCard
          label="White Frame Orders"
          value={fmtNum(data.conversions.white_frame)}
          icon={<Frame className="h-4 w-4" />}
        />
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">
        <KpiCard
          ...
          value={fmtNum(data.customers?.total_customers ?? 0)}
          icon={<Users className="h-4 w-4" />}
          accent="info"
        />
        <KpiCard
          label="Returning Customers"
          value={fmtNum(data.customers?.returning_customers ?? 0)}
          icon={<Repeat className="h-4 w-4" />}
          accent="success"
        />
        <KpiCard
          label="New Customers (30d)"
          value={fmtNum(data.customers?.new_customers ?? 0)}
          icon={<Sparkles className="h-4 w-4" />}
          accent="success"
        />
        <KpiCard
          label="Weekly Revenue"
          value={fmtEGP(data.revenue_week)}
          icon={<Calendar className="h-4 w-4" />}
          accent="success"
        />
        <KpiCard
          label="Total Revenue"
          value={fmtEGP(data.revenue_total)}
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 lg:grid-cols-7">
        ...
        <KpiCard
          label="Processing"
          value={fmtNum(data.orders_processing ?? 0)}
        ...
        <KpiCard
          label="Printed"
          value={fmtNum(data.orders_printed ?? 0)}
        ...
        <KpiCard
          label="Shipped"
          value={fmtNum(data.orders_shipped ?? 0)}
        ...
        <KpiCard
          label="Delivered"
          value={fmtNum(data.orders_delivered ?? 0)}
          icon={<CheckCircle2 className="h-4 w-4" />}
          accent="success"
          onClick={goto("orders")}
        />
      ...
      <Panel title="AI Business Insights" icon={<Lightbulb className="h-4 w-4 text-amber-300" />}>
        {insights.length === 0 ? (
          <div className="text-xs text-muted-foreground">
            Insights will appear once you have more sales data.
          </div>
        ) : (
          <ul className="grid gap-2 sm:grid-cols-2">
            {insights.map((s, i) => (
              <li
                key={i}
                className="rounded-sm border border-border/60 bg-background/40 p-3 text-xs leading-relaxed"
              >
                <span className="mr-2 inline-flex items-center rounded-sm bg-amber-500/15 px-1.5 py-0.5 text-[10px] uppercase tracking-widest text-amber-300">
                  Insight
                </span>
                {s}
              </li>
            ...
            <ul className="divide-y divide-border/60 text-sm">
              {(data.top_sizes ?? []).map((s) => (
                <li key={s.size} className="flex items-center justify-between py-2">
                  ...
                  <span className="text-[11px] text-muted-foreground">
                    {fmtNum(s.orders)} orders · {fmtEGP(s.revenue)}
                  </span>
                </li>
              ))}
            </ul>
          ...
          {(data.lowest_performing ?? []).length === 0 ? (
            ...
            <ul className="divide-y divide-border/60 text-sm">
              {(data.lowest_performing ?? []).slice(0, 6).map((p) => (
                <li
                  key={p.id}
                  data-testid="lowest-performing-row"
                ...
                >
                  ...
                  <span className="rounded-sm bg-rose-500/15 px-2 py-0.5 text-[11px] text-rose-200">
                    {fmtNum(p.views_count)} views · 0 sales
                  </span>
                </li>
              ))}
            </ul>
            ...
            <ul className="divide-y divide-border/60 text-sm">
              {(data.top_cities ?? []).slice(0, 8).map((c) => (
                <li key={c.city} className="flex items-center justify-between py-2">
                  ...
                  <span className="text-[11px] text-muted-foreground">
                    {fmtNum(c.orders)} · {fmtEGP(c.revenue)}
                  </span>
                </li>
              ))}
            </ul>
            ...
            <ul className="divide-y divide-border/60 text-sm">
              {(data.top_returning ?? []).map((c, i) => (
                <li key={i} className="flex items-center justify-between py-2">
                  ...
                  <span className="text-[11px] text-muted-foreground">
                    {fmtNum(c.orders)}× · {fmtEGP(c.spent)}
                  </span>
                </li>
              ))}
            </ul>
          ...
          {buildLiveFeed(data)
            ...
            .map((a, i) => (
              <li key={i} className="flex items-center gap-3 py-2">
                <span className={`grid h-7 w-7 place-items-center rounded-full ${a.tone}`}>
                  {a.icon}
                </span>
            ...
            <ul className="divide-y divide-border/60 text-sm">
              {data.recent_orders.map((o) => (
                <li key={o.id} className="flex items-center gap-2 py-2">
                  ...
                  {statusPill(o.status)}
                  <span className="hidden w-16 text-right text-[10px] text-muted-foreground sm:inline">
                ...
                <li key={c.id} className="flex items-center justify-between gap-2 py-2">
                  <div className="min-w-0">
                    ...
                    <div className="text-[11px] text-muted-foreground">
                      {fmtNum(c.image_count)} image{c.image_count === 1 ? "" : "s"} ·{" "}
                      {fmtEGP(c.total_price)}
                    </div>
                  </div>
                  <span className="text-[10px] text-muted-foreground">{fmtTime(c.created_at)}</span>
                </li>
...
function buildInsights(d: Dashboard): string[] {
  ...
  if (sizes[0])
    out.push(`${sizes[0].size} is your best-selling size (${fmtNum(sizes[0].orders)} orders).`);
  const govs = d.top_governorates ?? [];
  if (govs[0])
    out.push(
      `${govs[0].governorate} leads in revenue with ${fmtEGP(govs[0].revenue)} across ${fmtNum(govs[0].orders)} orders.`,
    );
  const top = d.top_selling?.[0];
  if (top && top.sales_count > 1)
    out.push(`${top.title} is your top performer with ${fmtNum(top.sales_count)} sales.`);
  ...
  if (cust && cust.total_customers > 0) {
    ...
    out.push(
      `${rate}% of customers have ordered more than once — focus on retention to grow this.`,
    );
  }
  const lowest = d.lowest_performing ?? [];
  if (lowest[0])
    out.push(
      `${lowest[0].title} has ${fmtNum(lowest[0].views_count)} views but no sales — consider better pricing or imagery.`,
    );
  const noRes = d.no_result_searches ?? [];
  if (noRes[0])
    out.push(
      `Customers are searching for “${noRes[0].q}” (${fmtNum(noRes[0].c)}× with no results) — add matching products.`,
    );
  const conv = d.conversions;
  if (conv?.custom_design > 0)
    out.push(
      `Custom design has ${fmtNum(conv.custom_design)} orders — promote this premium service more.`,
    );
  if (d.orders_cancelled > 0 && d.orders_total > 0) {
    const r = ((d.orders_cancelled / d.orders_total) * 100).toFixed(1);
    if (Number(r) > 5)
      out.push(`Cancellation rate is ${r}% — review checkout and shipping to reduce drop-off.`);
  }
    ...
    <Panel
      ...
      action={
        <button
          onClick={save}
          disabled={saving}
          className="inline-flex items-center gap-1 rounded-sm border border-border px-2 py-1 text-[10px] uppercase tracking-widest hover:bg-accent disabled:opacity-50"
        >
    ...
    >
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        <div className="grid grid-cols-2 gap-3 text-sm">
          ...
          <Stat
            label="Profit / Order"
            value={fmtEGP(perOrder)}
            tone={perOrder >= 0 ? "success" : "danger"}
          />
        </div>
      </div>
    </Panel>
...
async function exportDashboardPdf(d: Dashboard) {
  try {
    ...
    autoTable(doc, {
      ...
      body: [
        ...
        [
          ...
          `${d.orders_pending} / ${d.orders_processing ?? 0} / ${d.orders_printed ?? 0}`,
        ],
        [
          "Shipped / Delivered / Cancelled",
          `${d.orders_shipped ?? 0} / ${d.orders_delivered ?? 0} / ${d.orders_cancelled}`,
        ],
        ["Total Customers", fmtNum(d.customers?.total_customers ?? 0)],
        ["Returning Customers", fmtNum(d.customers?.returning_customers ?? 0)],
        [
          "Visitors (today / month / total)",
          `${d.visitors_today} / ${d.visitors_month} / ${d.visitors_total}`,
        ],
      ],
      theme: "grid",
    ...
    autoTable(doc, {
      ...
      body: (d.top_categories ?? [])
        .slice(0, 10)
        .map((c) => [c.name, fmtNum(c.sales), fmtNum(c.views)]),
      theme: "striped",
    });
    autoTable(doc, {
      head: [["Top Governorate", "Orders", "Revenue"]],
      body: (d.top_governorates ?? [])
        .slice(0, 10)
        .map((g) => [g.governorate, fmtNum(g.orders), fmtEGP(g.revenue)]),
      theme: "striped",
    });
