...
import {
  Download, Printer, TrendingUp, ShoppingBag, Users, RotateCcw, AlertTriangle, MapPin, Package,
} from "lucide-react";
...
function range(preset: string, fromISO?: string, toISO?: string) {
  ...
  if (preset === "today") return { from: startOfDay(now), to: new Date(startOfDay(now).getTime() + 86400000) };
  if (preset === "yesterday") {
    const s = new Date(startOfDay(now).getTime() - 86400000);
    return { from: s, to: new Date(s.getTime() + 86400000) };
  }
  if (preset === "7d") return { from: new Date(startOfDay(now).getTime() - 6 * 86400000), to: new Date(startOfDay(now).getTime() + 86400000) };
  if (preset === "30d") return { from: new Date(startOfDay(now).getTime() - 29 * 86400000), to: new Date(startOfDay(now).getTime() + 86400000) };
  if (preset === "month") return { from: new Date(now.getFullYear(), now.getMonth(), 1), to: new Date(startOfDay(now).getTime() + 86400000) };
  return { from: new Date(startOfDay(now).getTime() - 13 * 86400000), to: new Date(startOfDay(now).getTime() + 86400000) };
}
...
export function ReportsTab({ onNavigate }: { onNavigate?: (tab: string) => void }) {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.rpc("admin_dashboard" as never, {
        p_from: rf.toISOString(), p_to: rt.toISOString(),
      } as never);
  ...
  const exportCSVFile = () => {
    ...
    a.href = url; a.download = `brwaz-orders-${preset}.csv`; a.click();
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-2">
        {PRESETS.map((p) => (
          <button key={p.key} onClick={() => setPreset(p.key)}
            className={cn("rounded-sm border px-3 py-1.5 text-xs uppercase tracking-widest",
              preset === p.key ? "border-primary bg-primary/10 text-primary" : "border-border text-muted-foreground hover:bg-accent")}>
            {p.label}
          </button>
        ))}
        ...
        <div className="ml-auto flex gap-2">
          <button onClick={exportCSVFile} className="inline-flex items-center gap-1 rounded-sm border border-border px-3 py-1.5 text-xs uppercase tracking-widest hover:bg-accent">
            <Download className="h-3.5 w-3.5" /> CSV
          </button>
          <button onClick={exportCSV} className="inline-flex items-center gap-1 rounded-sm border border-border px-3 py-1.5 text-xs uppercase tracking-widest hover:bg-accent">
            <Download className="h-3.5 w-3.5" /> Excel
          </button>
          <button onClick={() => window.print()} className="inline-flex items-center gap-1 rounded-sm border border-border px-3 py-1.5 text-xs uppercase tracking-widest hover:bg-accent">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
        <Stat icon={<TrendingUp className="h-4 w-4" />} label="Revenue" value={`${Math.round(revenueRange)} EGP`} tone="emerald" />
      ...
      <Section title="Customers" icon={<Users className="h-4 w-4" />}>
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
          ...
          <Stat label="Returning %" value={
            customers.total_customers ? `${Math.round((customers.returning_customers ?? 0) * 100 / customers.total_customers)}%` : "0%"
          } />
        </div>
        <TopTable title="Top Returning Customers" rows={list("top_returning")}
          cols={[["name","Name"],["phone","Phone"],["gov","Gov"],["orders","Orders"],["spent","Spent"]]} />
      </Section>
      ...
      <Section title="Products" icon={<Package className="h-4 w-4" />}>
        <div className="grid gap-3 md:grid-cols-2">
          <TopTable title="Best Sellers" rows={list("top_selling")} cols={[["title","Title"],["sales_count","Sales"]]} />
          <TopTable title="Most Viewed" rows={list("top_viewed")} cols={[["title","Title"],["views_count","Views"]]} />
          <TopTable title="Most Added to Cart" rows={list("top_cart")} cols={[["title","Title"],["cart_adds_count","Adds"]]} />
          <TopTable title="Most Wishlisted" rows={list("top_wishlisted")} cols={[["title","Title"],["wishlist_count","Wishes"]]} />
          <TopTable title="Top Sizes" rows={list("top_sizes")} cols={[["size","Size"],["orders","Orders"],["revenue","Revenue"]]} />
          <TopTable title="Lowest Performing" rows={list("lowest_performing")} cols={[["title","Title"],["views_count","Views"],["sales_count","Sales"]]} />
        </div>
      </Section>
      ...
      <Section title="Geography" icon={<MapPin className="h-4 w-4" />}>
        <div className="grid gap-3 md:grid-cols-2">
          <TopTable title="Top Governorates" rows={list("top_governorates")} cols={[["governorate","Governorate"],["orders","Orders"],["revenue","Revenue"]]} />
          <TopTable title="Top Cities" rows={list("top_cities")} cols={[["city","City"],["orders","Orders"],["revenue","Revenue"]]} />
        </div>
      </Section>
...
function Section({ title, icon, children }: { title: string; icon?: React.ReactNode; children: React.ReactNode }) {
  return (
    <div className="rounded-sm border border-border bg-background p-4">
      <div className="mb-3 flex items-center gap-2 text-display text-lg">{icon}{title}</div>
      <div className="space-y-3">{children}</div>
    </div>
  );
}

function Stat({ label, value, tone, icon }: { label: string; value: string | number; tone?: "emerald" | "blue" | "red"; icon?: React.ReactNode }) {
  return (
    <div className={cn(
...
function ClickStat({ label, value, onClick }: { label: string; value: string | number; onClick: () => void }) {
  return (
    <button onClick={onClick} className="rounded-sm border border-border bg-background p-3 text-left transition hover:border-primary hover:bg-primary/5">
...
function TopTable({ title, rows, cols }: { title: string; rows: Row[]; cols: [string, string][] }) {
  return (
    <div className="rounded-sm border border-border">
      <div className="border-b border-border bg-muted/30 px-2 py-1.5 text-[10px] font-semibold uppercase tracking-widest text-muted-foreground">{title}</div>
      {rows.length === 0 ? (
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="text-[10px] uppercase tracking-widest text-muted-foreground">
              {cols.map(([k, l]) => <th key={k} className="p-2 text-left">{l}</th>)}
            </tr>
          </thead>
          <tbody>
            {rows.slice(0, 10).map((r, i) => (
              <tr key={i} className="border-t border-border">
                {cols.map(([k]) => {
                  const v = r[k];
                  const val = typeof v === "number" ? (Math.abs(v) > 100 && k.includes("evenue") ? `${Math.round(v)} EGP` : String(v)) : (v == null ? "—" : String(v));
                  return <td key={k} className="p-2 text-xs">{val}</td>;
                })}
              </tr>
            ))}
          </tbody>
