...
type ReviewStatus =
  "ready" | "needs_edit" | "needs_replace" | "needs_seo" | "low_quality" | "draft";
...
export function SubCategoryReviewManager({
  ...
  const {
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("posters")
        .select(
          "id,title,image_url,original_url,category_id,price,hidden,featured,seo_title,seo_description,alt_text,tags,slug,review_status,created_at,sales_count,views_count,trending,edit_settings",
        )
  ...
  const setHidden = (ids: string[], hidden: boolean) => {
    patchPoster(ids, { hidden }).then(() =>
      toast.success(hidden ? `Hidden ${ids.length}` : `Shown ${ids.length}`),
    );
  };
  ...
  const setReview = (ids: string[], review_status: ReviewStatus) => {
    patchPoster(ids, { review_status }).then(() =>
      toast.success(`Marked ${ids.length} as ${REVIEW_LABELS[review_status]}`),
    );
  };
  const softDelete = (ids: string[]) => {
    if (
      !confirm(
        `Move ${ids.length} poster${ids.length === 1 ? "" : "s"} to hidden? They stay in the database and won't be shown on the storefront. Old orders keep working.`,
      )
    )
      return;
  ...
  const runAiSeoSingle = async (p: Poster) => {
    ...
    try {
      ...
      if (Object.keys(patch).length === 0)
        return toast.info("Nothing to update — fields already filled");
      ...
      <DialogContent className="max-h-[95vh] max-w-[95vw] overflow-hidden p-0 flex flex-col">
        ...
        <div className="border-b border-border bg-muted/20 p-3 flex flex-wrap items-center gap-2">
          ...
          <SelectFilter
            ...
            onChange={(v) => {
              setFilters((f) => ({ ...f, visibility: v as Filters["visibility"] }));
              setPage(1);
            }}
            options={[
              ["all", "All visibility"],
          ...
          <SelectFilter
            ...
            onChange={(v) => {
              setFilters((f) => ({ ...f, review: v as Filters["review"] }));
              setPage(1);
            }}
            options={[
              ["all", "All statuses"],
          ...
          <SelectFilter
            value={filters.seo}
            onChange={(v) => {
              setFilters((f) => ({ ...f, seo: v as Filters["seo"] }));
              setPage(1);
            }}
            options={[
              ["all", "All SEO"],
              ["with_seo", "SEO complete"],
          ...
          <SelectFilter
            value={filters.sort}
            onChange={(v) => {
              setFilters((f) => ({ ...f, sort: v as Filters["sort"] }));
              setPage(1);
            }}
            options={[
              ["newest", "Newest first"],
        ...
        {selected.size > 0 && (
          <div className="flex flex-wrap items-center gap-2 border-b border-border bg-primary/5 p-3 text-sm">
            ...
            <button
              onClick={() => setReview([...selected], "ready")}
              className="inline-flex items-center gap-1 rounded-sm border border-border px-2 py-1 text-[10px] uppercase tracking-widest hover:bg-accent"
            >
              <CheckCircle2 className="h-3 w-3" /> Mark Ready
            </button>
            <button
              onClick={() => setReview([...selected], "needs_edit")}
              className="inline-flex items-center gap-1 rounded-sm border border-border px-2 py-1 text-[10px] uppercase tracking-widest hover:bg-accent"
            >
              <AlertTriangle className="h-3 w-3" /> Needs Edit
            </button>
            <button
            ...
            <button
              ...
              className="inline-flex items-center gap-1 rounded-sm border border-destructive px-2 py-1 text-[10px] uppercase tracking-widest text-destructive hover:bg-destructive/10"
            >
        ...
        {detail && (
          <PosterDetailModal
            ...
            onReview={(s) => {
              setReview([detail.id], s);
              setDetail({ ...detail, review_status: s });
            }}
            onHide={() => {
              setHidden([detail.id], !detail.hidden);
              setDetail({ ...detail, hidden: !detail.hidden });
            }}
            onDelete={() => {
              softDelete([detail.id]);
              setDetail(null);
            }}
...
function StatChip({
  ...
  label: string;
  value: number;
  tone?: "default" | "emerald" | "muted" | "amber" | "red" | "blue";
}) {
  const toneClass = {
  ...
  return (
    <span
      className={cn(
        "inline-flex items-center gap-1 rounded-sm border px-2 py-1 text-[10px] uppercase tracking-widest",
        toneClass,
      )}
    >
    ...
    >
      ...
      <div className="mt-2 space-y-1.5">
        ...
        <div className="flex flex-wrap gap-1">
          <span
            className={cn(
              "rounded border px-1.5 py-0.5 text-[9px] uppercase tracking-widest",
        ...
        <div className="flex flex-wrap gap-1">
          ...
          <button
            ...
            className="rounded-sm border border-primary/40 bg-primary/5 p-1 text-primary hover:bg-primary/10"
          >
            <Crop className="h-3 w-3" />
          </button>
          <label
            title="Replace image"
            className="cursor-pointer rounded-sm border border-border p-1 hover:bg-accent"
          >
      ...
      <DialogContent className="max-h-[95vh] max-w-4xl overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            {p.hidden && (
              <span className="rounded bg-black/70 px-1.5 py-0.5 text-[10px] uppercase tracking-widest text-white">
                Hidden
              </span>
            )}
          </DialogTitle>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <div className="space-y-3 text-sm">
            ...
            <div className="flex flex-wrap gap-2 pt-2">
              ...
              <button
                onClick={onAiSeo}
                className="inline-flex items-center gap-1 rounded-sm border border-primary/40 bg-primary/10 px-3 py-1.5 text-xs uppercase tracking-widest text-primary hover:bg-primary/20"
              >
              ...
              <button
                onClick={onClose}
                className="ml-auto inline-flex items-center gap-1 rounded-sm border border-border px-3 py-1.5 text-xs uppercase tracking-widest hover:bg-accent"
              >
