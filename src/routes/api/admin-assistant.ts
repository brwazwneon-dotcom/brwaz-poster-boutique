...
type HomepageSection = Record<string, JsonValue | undefined> & {
  key?: string;
  visible?: boolean;
  title_ar?: string;
  title_en?: string;
...
function safe<Args extends unknown[], Result extends object>(
  name: string,
  ...
  return async (...args: Args) => {
    try {
      ...
      return {
        ok: false,
        friendly: "حصلت مشكلة أثناء تنفيذ الطلب. جرّب مرة أخرى أو افتح Error Logs لمعرفة التفاصيل.",
        friendly_en:
      ...
      POST: async ({ request }) => {
        ...
        const tools = {
          ...
          queryTopProducts: tool({
            ...
            execute: safe(
              "queryTopProducts",
              async ({ by, limit }: { by: string; limit: number }) => {
          ...
          queryRecentOrders: tool({
            ...
            execute: safe(
              "queryRecentOrders",
              async ({ limit, status }: { limit: number; status?: string }) => {
                let q = supabaseAdmin
                  .from("orders")
                  .select(
          ...
          queryNotifications: tool({
            ...
            execute: safe(
              "queryNotifications",
              async ({ unreadOnly, limit }: { unreadOnly: boolean; limit: number }) => {
                let q = supabaseAdmin
          ...
          queryOrdersNeedConfirmation: tool({
            ...
            execute: safe("queryOrdersNeedConfirmation", async ({ limit }: { limit: number }) => {
              const { data, error } = await supabaseAdmin
                .from("orders")
                .select(
                  "id,order_number,customer_name,phone,governorate,status,total_price,poster_title,quantity,created_at",
                )
          ...
          queryAbandonedCarts: tool({
            ...
            execute: safe("queryAbandonedCarts", async ({ limit }: { limit: number }) => {
              const { data, error } = await supabaseAdmin.rpc("admin_abandoned_orders", {
                p_limit: limit,
              });
              if (error) throw error;
              return { carts: data ?? [] };
          ...
          queryCustomersSegment: tool({
            ...
            execute: safe(
              "queryCustomersSegment",
              async ({ segment, limit }: { segment: string; limit: number }) => {
                const { data, error } = await supabaseAdmin
                  .from("orders")
          ...
          querySalesReport: tool({
            ...
            execute: safe("querySalesReport", async ({ period }: { period: string }) => {
              ...
              for (const r of rows) {
                ...
                if (r.poster_title)
                  byProduct[r.poster_title] = (byProduct[r.poster_title] ?? 0) + (r.quantity ?? 1);
                byStatus[r.status] = (byStatus[r.status] ?? 0) + 1;
              }
              const topProducts = Object.entries(byProduct)
                .sort((a, b) => b[1] - a[1])
                .slice(0, 10);
              const topGov = Object.entries(byGov)
                .sort((a, b) => b[1] - a[1])
                .slice(0, 10);
              return {
                period,
          ...
          queryErrorLogsSummary: tool({
            description:
              "ملخص Error Logs خلال آخر 24 ساعة (عدد حسب المستوى، أعلى الأخطاء تكرارًا).",
            inputSchema: z.object({}),
          ...
          queryPerformanceSummary: tool({
            ...
            execute: safe("queryPerformanceSummary", async () => {
              ...
              for (const r of rows) {
                ...
                g.count += 1;
                g.sum += Number(r.value_ms);
                g.max = Math.max(g.max, Number(r.value_ms));
                grouped[r.metric] = g;
              }
              const summary = Object.fromEntries(
                Object.entries(grouped).map(([k, v]) => [
                  k,
                  { avg: Math.round(v.sum / v.count), max: Math.round(v.max), samples: v.count },
                ]),
              );
              ...
              async ({
                ...
                if (hidden && !confirm) {
                  return {
                    ...
                    prompt: "هل تريد فعلًا إخفاء هذا البوستر؟",
                    posterId,
                  };
                }
          ...
          setPosterTrending: tool({
            ...
            execute: safe(
              "setPosterTrending",
              async ({ posterId, trending }: { posterId: string; trending: boolean }) => {
                const { error } = await supabaseAdmin
                  .from("posters")
                  .update({ trending })
                  .eq("id", posterId);
            ...
            execute: safe(
              ...
              async ({
                ...
                if (risky.includes(status.toLowerCase()) && !confirm) {
                  return {
                    needsConfirmation: true,
                    prompt: `تأكيد تغيير حالة الأوردر إلى ${status}؟`,
                    orderId,
                    status,
                  };
                }
          ...
          updateHomepageSection: tool({
            ...
            execute: safe(
              ...
              async ({
                ...
                const list: HomepageSection[] = Array.isArray(currentValue)
                  ? [...(currentValue as HomepageSection[])]
                  : [];
                const idx = list.findIndex((section) => section.key === key);
                if (idx === -1)
                  return { updated: false, message: `لم يتم العثور على قسم بالمفتاح ${key}` };
                ...
                const { error } = await supabaseAdmin
                  .from("site_settings")
                  .upsert({ key: "homepage_sections_v1", value: list }, { onConflict: "key" });
                if (error) throw error;
          ...
          generateProductDescription: tool({
            description: "توليد وصف احترافي بالعربية لمنتج (بوستر/إطار). يرجع الوصف كنص فقط.",
            inputSchema: z.object({
              title: z.string(),
          ...
          queryVisitorStats: tool({
            description:
              "إحصائيات الزوار (today/yesterday/week) — visitors، unique، pageviews، أهم المصادر.",
            inputSchema: z.object({
            ...
            execute: safe("queryVisitorStats", async ({ period }: { period: string }) => {
              ...
              let q = supabaseAdmin
                ...
                .select("visitor_id,path,source,device,created_at")
                .gte("created_at", start.toISOString())
                .limit(5000);
              if (end) q = q.lt("created_at", end.toISOString());
              const { data, error } = await q;
          ...
          queryLiveVisitors: tool({
            ...
            execute: safe("queryLiveVisitors", async () => {
              ...
              return {
                ...
                topPages: Object.entries(currentPages)
                  .sort((a, b) => b[1] - a[1])
                  .slice(0, 10),
              };
            }),
          ...
          querySalesFunnel: tool({
            ...
            execute: safe("querySalesFunnel", async ({ period }: { period: string }) => {
              ...
              const addedToCart = new Set(
                (cartEvents ?? [])
                  .filter((r) => /add/i.test(r.event ?? ""))
                  .map((r) => r.visitor_id),
              );
              const checkoutStarted = new Set(
                (cartEvents ?? [])
                  .filter((r) => /check/i.test(r.event ?? ""))
                  .map((r) => r.visitor_id),
              );
              ...
              for (let i = 0; i < funnel.length - 1; i++) {
                ...
                rates.push({
                  from: funnel[i].step,
                  to: funnel[i + 1].step,
                  rate: a > 0 ? Math.round((b / a) * 1000) / 10 : 0,
                });
              }
          ...
          queryCartInsights: tool({
            ...
            execute: safe("queryCartInsights", async ({ days }: { days: number }) => {
              ...
              const [{ data: cart }, { data: views }] = await Promise.all([
                supabaseAdmin
                  .from("visitor_cart_events")
                  .select("poster_id,size,event")
                  .gte("created_at", iso)
                  .limit(20000),
                supabaseAdmin
                  .from("analytics_poster_events")
                  .select("poster_id,event_type")
                  .gte("created_at", iso)
                  .eq("event_type", "view")
                  .limit(20000),
              ]);
              ...
              for (const v of views ?? [])
                if (v.poster_id) viewsBy[v.poster_id] = (viewsBy[v.poster_id] ?? 0) + 1;
              ...
              for (const [pid, vc] of Object.entries(viewsBy)) {
                ...
                if (vc >= 10 && (ac === 0 || vc / Math.max(ac, 1) >= 20))
                  problematic.push({ poster_id: pid, views: vc, adds: ac });
              }
              problematic.sort((a, b) => b.views - a.views);
              const topProblem = problematic.slice(0, 10);
              const posterIds = [
                ...new Set([
                  ...Object.keys(byPoster).slice(0, 10),
                  ...topProblem.map((p) => p.poster_id),
                ]),
              ];
              ...
              const topAdded = Object.entries(byPoster)
                .sort((a, b) => b[1] - a[1])
                .slice(0, 10)
                .map(([id, adds]) => ({ id, title: titles[id] ?? id, adds }));
              return {
                days,
                topSizes: Object.entries(bySize)
                  .sort((a, b) => b[1] - a[1])
                  .slice(0, 10),
                topAddedProducts: topAdded,
                highViewsLowCart: topProblem.map((p) => ({
                  ...p,
                  title: titles[p.poster_id] ?? p.poster_id,
                })),
              };
            }),
          }),
          ...
          suggestConversionImprovements: tool({
            ...
            inputSchema: z.object({
              biggestDropStep: z
                .string()
                .describe(
                  "اسم الخطوة الأضعف من الفانل (مثلاً 'Add to Cart' أو 'Checkout Started')",
                ),
              context: z.string().optional(),
            }),
            execute: safe(
              "suggestConversionImprovements",
              async ({
                biggestDropStep,
                context,
              }: {
                ...
                const key =
                  Object.keys(catalog).find((k) =>
                    biggestDropStep.toLowerCase().includes(k.toLowerCase()),
                  ) ?? "Add to Cart";
          ...
          diagnoseError: tool({
            ...
            execute: safe("diagnoseError", async ({ errorId }: { errorId?: string }) => {
              ...
              else q = q.in("level", ["error", "critical", "warn"]).is("status", null);
              const { data, error } = await q;
              if (error) throw error;
              const err = data?.[0];
              ...
              if (/fetch|network|timeout/i.test(msg))
                suggested.push("إعادة المحاولة (Retry)", "التحقق من اتصال المصدر الخارجي");
              if (/upload|image/i.test(msg))
                suggested.push("Re-run image processing", "التأكد من حجم الملف");
              if (/ai|gemini|openrouter|rate/i.test(msg))
                suggested.push("Reset مفتاح AI Cooldown", "تبديل Provider fallback");
              if (/permission|denied|forbidden|rls/i.test(msg))
                suggested.push("مراجعة سياسات RLS", "مراجعة صلاحيات المستخدم");
              if (!suggested.length)
                suggested.push("افتح Error Logs للتفاصيل", "أعد تشغيل العملية");
              return {
                error: {
                  id: err.id,
                  message: msg,
          ...
          runSafeAutoFix: tool({
            ...
            execute: safe(
              "runSafeAutoFix",
              async ({ kind, confirm }: { kind: string; confirm: boolean }) => {
                if (!confirm) {
                ...
                if (kind === "reset_all_gemini_cooldowns") {
                  const { resetGeminiKeyCooldown, getGeminiKeysStatus } =
                  ...
                  for (const s of status) {
                    ...
                    if (!label) continue;
                    try {
                      if (resetGeminiKeyCooldown(label)) reset++;
                    } catch {
                      /* ignore */
                    }
                  }
                ...
                if (kind === "recalculate_poster_counters") {
                  ...
                  return {
                    ...
                    kind,
                    queued: true,
                    note: "تم جدولة إعادة الحساب في الخلفية.",
                  };
                }
          ...
          runPerformanceFix: tool({
            ...
            inputSchema: z.object({
              kind: z.enum([
                "disable_hidden_home_sections",
                "reduce_homepage_items_count",
                "clear_perf_metrics",
              ]),
            ...
            execute: safe(
              ...
              async ({
                ...
                if (!confirm)
                  return {
                    needsConfirmation: true,
                    prompt: `تأكيد تنفيذ إصلاح الأداء: ${kind}؟`,
                    kind,
                  };
          ...
          markCartRecoveryAction: tool({
            ...
            execute: safe(
              ...
              async ({
                ...
                const { error } = await supabaseAdmin
                  .from("admin_notifications")
                  .insert(notification as unknown as never);
                if (error) throw error;
