...
BEGIN
  ...
  WITH
    ...
    v_month AS (
      SELECT COUNT(DISTINCT visitor_id) AS c FROM public.analytics_visits WHERE created_at >= month_start
    ),
    v_total AS (
    ...
    chart_days AS (
      SELECT generate_series(chart_start::date, today_start::date, interval '1 day')::date AS d
    ),
    orders_by_day AS (
      SELECT date_trunc('day', created_at)::date AS d,
             COUNT(*)::int AS orders,
             COALESCE(SUM(total_price), 0)::numeric AS revenue
      FROM public.orders WHERE created_at >= chart_start
      GROUP BY 1
    ),
    visitors_by_day AS (
      SELECT date_trunc('day', created_at)::date AS d,
             COUNT(DISTINCT visitor_id)::int AS visitors
      FROM public.analytics_visits WHERE created_at >= chart_start
      GROUP BY 1
    ),
    chart AS (
      SELECT
        to_char(cd.d, 'YYYY-MM-DD') AS day,
    ...
    recent_customers AS (
      ...
      ORDER BY lower(customer_name||'|'||COALESCE(phone,'')), created_at DESC
      LIMIT 8
    ),
    recent_reviews AS (
    ...
    recent_custom AS (
      SELECT id, customer_name, total_price, image_count, created_at
...
AS $$
  SELECT CASE
    WHEN private.has_role(auth.uid(), 'admin'::app_role)
    THEN (SELECT COUNT(DISTINCT visitor_id)::int FROM public.analytics_visits WHERE created_at > now() - interval '5 minutes')
    ELSE 0
  END;
$$;
