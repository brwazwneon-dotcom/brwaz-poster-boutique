...
-- Seed default profit cost settings if missing
INSERT INTO public.site_settings (key, value)
VALUES ('profit_costs', jsonb_build_object(
  'product_cost', 0,
  'packaging_cost', 0,
  'shipping_cost', 0,
  'advertising_cost', 0
...
-- Replace admin_dashboard with optional date range + new metrics
DROP FUNCTION IF EXISTS public.admin_dashboard();
DROP FUNCTION IF EXISTS public.admin_dashboard(timestamptz, timestamptz);
...
BEGIN
  ...
  WITH
    ...
    o_range AS (SELECT COUNT(*)::int AS c, COALESCE(SUM(total_price), 0)::numeric AS rev FROM public.orders WHERE created_at >= range_from AND created_at < range_to),
    o_status AS (
      SELECT
    ...
    -- customers identified by phone (fallback name) over all orders
    cust_keys AS (
      SELECT
    ...
    orders_by_day AS (
      ...
      FROM public.orders WHERE created_at >= range_from AND created_at < range_to GROUP BY 1
    ),
    visitors_by_day AS (
    ...
    top_sizes AS (
      ...
      FROM public.orders WHERE size IS NOT NULL AND size <> ''
    ...
    -- "cities" derived from address line (first segment); falls back to governorate
    top_cities AS (
      SELECT
  ...
  SELECT jsonb_build_object(
    'range', jsonb_build_object(
      'from', range_from, 'to', range_to, 'days', days_count
    ),
